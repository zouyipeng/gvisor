...
void restore_state(struct sysmsg *sysmsg, struct thread_context *ctx,
  ...
  // in the guest context's PSTATE. Gated on sysmsg->debug_enabled so the MRS
  // of VBAR_EL0_FGT doesn't tax the return path in production.
  if (sysmsg->debug_enabled) {
...
void __syshandler(struct sysmsg *sysmsg) {
  ...
  // via sysmsg->debug (see subprocess.go switchToApp). Gated on
  // sysmsg->debug_enabled so the atomic RMW doesn't tax the fast path in
  // production.
  if (sysmsg->debug_enabled) {
