...
func (s *subprocess) switchToApp(c *platformContext, ac *arch.Context64) (isSyscall bool, shouldPatchSyscall bool, at hostarch.AccessType, err *platform.ContextError) {
	...
	// can be correlated with kernel prints. Only done when debug logging is on
	// so the per-syscall RLock + shared Debug read don't tax the hot path.
	if log.IsLogging(log.Debug) {
		var fgtPid int32
...
func (s *subprocess) createSysmsgThread() error {
	...
	if log.IsLogging(log.Debug) {
		// Enable the stub-side FEAT_FGT diagnostics (entry counters and VBAR
		// sampling). They add per-syscall shared-memory traffic to the fast
		// path, so they are only turned on when debug logging is requested.
		sysThread.msg.DebugEnabled = 1
	}
